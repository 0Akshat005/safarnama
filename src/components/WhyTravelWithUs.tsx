...
export const WhyTravelWithUs: React.FC = () => {
  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="relative bg-black/40 backdrop-blur-2xl rounded-[2.5rem] p-8 sm:p-10 border border-white/15 shadow-2xl overflow-hidden text-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10">
          {FEATURES_DATA.map((feature) => (
            <div
              key={feature.id}
              className="flex items-center sm:items-start gap-4 p-4 rounded-2xl transition-all border border-white/10 bg-white/5 backdrop-blur-md hover:bg-white/10 hover:border-white/20"
            >
              {/* Circular Icon Badge */}
              <div className="w-12 h-12 rounded-full bg-[#2d5a3f] flex items-center justify-center shrink-0 shadow-lg border border-emerald-400/30">
              ...
              <div className="flex flex-col">
                ...
                <p className="text-slate-200 text-xs sm:text-sm mt-0.5 leading-relaxed font-normal">
                  {feature.description}
                </p>
              </div>
            </div>
